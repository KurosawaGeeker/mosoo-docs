...
<Note>
  The API only works with published Agents. Agent setup, Provider configuration, and Channel bindings are managed in Mosoo before you call these endpoints.
</Note>

<Columns cols={3}>
  <Card title="1. Publish an Agent" icon="bot">
    Configure the Agent in Mosoo, connect its Provider, and publish it so it receives an `agentId`.
  </Card>
  <Card title="2. Create a token" icon="key-round">
    Use a Human PAT for user-owned calls or an Organization Service token for backend jobs.
  </Card>
...
<Columns cols={2}>
  ...
  <Card title="Caller token" icon="badge-check">
    A Human PAT or Organization Service token. The token identifies the caller; it does not replace the Agent owner's runtime configuration.
  </Card>
...
- Start an Agent from your own backend when a ticket, order, alert, or job appears.
...
<Columns cols={2}>
  ...
  <Card title="Attach files" icon="paperclip" href="/api-reference/add-a-thread-file">
    Add, list, and remove Thread files. API-added files are attachments; Agent-produced files are artifacts.
  </Card>
...
- Runtime execution resolves the published Agent owner's configured capabilities.

<Card title="Understand tokens and access" icon="shield-check" horizontal href="/auth-and-access">
  Read how Human PATs, Organization Service tokens, Agent access modes, attribution, and owner execution fit together.
</Card>
