...
A Thread is the API conversation container for a published Agent. Creating one also starts the initial run.
...
The event log is the stable read surface for API consumers. It can include user messages, Agent message deltas, thinking deltas, tool status, file changes, usage updates, and run status.
...
<Columns cols={2}>
  <Card title="Create a Thread" icon="plus" href="/api-reference/create-a-thread-for-a-published-agent">
