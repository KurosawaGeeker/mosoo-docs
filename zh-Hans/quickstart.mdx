...
- Agent API Access 面板里的 Agent ID。
...
<Info>
  如果你是从 Agent 的 **API Access** 动作打开这份文档，就使用那里展示的 `agentId`。
</Info>

## 1. 创建 Thread

Thread 是已发布 Agent 的 API 对话容器。创建 Thread 也会启动初始 run。
...
事件日志是 API 调用方稳定的读取面。它可能包含用户消息、Agent 消息增量、thinking 增量、工具状态、文件变化、用量更新和 run 状态。
...
<Columns cols={2}>
  <Card title="创建 Thread" icon="plus" href="/api-reference/create-a-thread-for-a-published-agent">
